<script lang="ts">
	...
	import { COUNTRY_DATA, getCountryCode, getCountryName } from '$lib/utils/countries';
	...
	function loadSettings() {
		...
		country = $profile.country_code ? getCountryName($profile.country_code) : '';
	...
	async function handleSaveProfile() {
		...
		try {
			...
			const countryCode = country ? getCountryCode(country) : ($profile?.country_code || 'US');
			const region = country ? COUNTRY_DATA[country].region : 'north_america';
			...
			{#if activeTab === 'profile'}
				...
				<div class="card mb-6">
					...
					<div>
						...
						<CountrySelect
							bind:value={country}
							onSelect={(selected) => country = selected}
						/>
						<p class="text-sm dark:text-gray-300 mt-1">
							This helps us show you on the regional leaderboards. Type to search!
						</p>
					</div>
				</div>
