<script lang="ts">
	import { onMount } from 'svelte';
	import { goto } from '$app/navigation';
	import { Gamepad2, Sparkles } from 'lucide-svelte';

	onMount(() => {
		// Redirect to homepage after 3 seconds
		const timer = setTimeout(() => {
			goto('/');
		}, 3000);

		return () => clearTimeout(timer);
	});
</script>

<svelte:head>
	<title>Welcome Back - Blue Balls</title>
</svelte:head>

<div class="min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-primary via-purple-500 to-secondary animate-gradient">
	<div class="text-center">
		<div class="mb-8 animate-bounce">
			<div class="w-32 h-32 mx-auto rounded-full bg-white/20 backdrop-blur-sm flex items-center justify-center">
				<Gamepad2 size={64} class="text-white" />
			</div>
		</div>

		<div class="space-y-4 animate-fade-in">
			<h1 class="text-5xl md:text-6xl font-bold text-white drop-shadow-lg">
				Good to have your balls back!
			</h1>
			<p class="text-xl md:text-2xl text-white/90 flex items-center justify-center gap-2">
				<Sparkles size={24} class="animate-pulse" />
				Let's start playing
				<Sparkles size={24} class="animate-pulse" />
			</p>
		</div>

		<div class="mt-12">
			<div class="flex justify-center gap-2">
				<div class="w-2 h-2 bg-white rounded-full animate-bounce" style="animation-delay: 0ms"></div>
				<div class="w-2 h-2 bg-white rounded-full animate-bounce" style="animation-delay: 150ms"></div>
				<div class="w-2 h-2 bg-white rounded-full animate-bounce" style="animation-delay: 300ms"></div>
			</div>
			<p class="text-white/70 text-sm mt-4">Redirecting you to the game...</p>
		</div>
	</div>
</div>

<style>
	@keyframes gradient {
		0% { background-position: 0% 50%; }
		50% { background-position: 100% 50%; }
		100% { background-position: 0% 50%; }
	}

	.animate-gradient {
		background-size: 200% 200%;
		animation: gradient 3s ease infinite;
	}

	@keyframes fade-in {
		from {
			opacity: 0;
			transform: translateY(20px);
		}
		to {
			opacity: 1;
			transform: translateY(0);
		}
	}

	.animate-fade-in {
		animation: fade-in 0.8s ease-out;
	}
</style>
