-- Blue Balls Game - Complete Database Schema
...
CREATE TABLE points_history (
    ...
    source TEXT NOT NULL, -- 'game', 'friend_bonus', etc.
    created_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE email_campaigns (
    ...
    body TEXT NOT NULL,
    target_audience TEXT, -- 'all', 'free', 'mid', 'big', 'affiliates'
    sent_at TIMESTAMPTZ,
    created_at TIMESTAMPTZ DEFAULT NOW(),
...
CREATE TABLE ads (
    ...
    position TEXT NOT NULL, -- 'game_header', 'game_over', etc.
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- Create admin user (cheahboolim@gmail.com)
-- This will be handled by a separate migration after user signs up
...
RETURNS INTEGER AS $$
BEGIN
    -- Progressive leveling: Level = floor(sqrt(points / 100)) + 1
